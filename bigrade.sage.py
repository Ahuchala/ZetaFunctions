...
_sage_const_1 = Integer(1); _sage_const_4 = Integer(4); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0)
import itertools
...
k = ZZ/{p};
R = k[x_0..x_{n}];
I = ideal {str(tuple(poly_list))[:-_sage_const_2 ]});
...
s = f'''
R = QQ[x_0..x_{n},y_1..y_{num_poly}, Degrees=>{{{n+_sage_const_1 }:{{0,1}},{str([(_sage_const_1 ,-i) for i in d])[_sage_const_1 :-_sage_const_1 ].replace('(','{').replace(')','}')}}}];
F={sum([y_vars[i] * poly_list[i] for i in range(num_poly)])};
