...
s += ", Degrees=>{" + str(n+_sage_const_1 ) + ":{0,1},"
...
s += "F = "
for i in range(num_poly):
	s += "y_" + str(i+_sage_const_1 ) + "*(" + str(poly_list[i]) + ")+"
s = s[:-_sage_const_1 ]
s += ";"
s += "J = R/ideal jacobian F;"
# s += "for p from 0 to " + str(n+2) + " list hilbertFunction({p,"  + str(m) + "}, J);"
s += "for i from 0 to " + str(n-_sage_const_2 ) + " list toString basis({i,"  + str(m) + "}, J);"
s = s[:-_sage_const_1 ]
t = str(macaulay2(s))
