...
R = QQ[x_0..x_{n},y_1..y_{num_poly}, Degrees=>{{{n+1}:{{0,1}},
{str([(1,-i) for i in d])[1:-1].replace('(','{').replace(')','}')}}}];
toString basis({{{max_cohomology_pole_order },{m}}}, R)
'''
...
# maybe I lift along (xi df/dxi)?
# x^{u+v}g -> x^u M_v g

# = Sum (u+v)_i x^(u+v-ei)g_i + x^(u+v) dgi/dxi + Sum (u+v)_k x^(u+v-ek)g_k + x^(u+v) dgk/dyk
def reduce_by_v_matrix(v):
    if not v in v_matrix_dict.keys():
        
    return v_matrix_dict[v]


def new_reduce_griffiths_dwork(u,g):
    g_vec = vector(matrix(to_pn_basis(g)))
    # todo: speed up!
    while(pole_order(vector_to_monomial(u)))>max_cohomology_pole_order:
        print(u)
