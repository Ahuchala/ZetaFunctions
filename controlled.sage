...
if use_macaulay:
    print('computing lifts of degree ' + str((n-1)* fdegree - n+d) + ' using macaulay')
    s = ''
    ...
    if n == 3:
        ...
        toString basis(''' + str((n-1)* fdegree - n+d) + ''',R), toString (basis(''' + str((n-1)* fdegree - n+d) + ''',R) // M)
        ''')
    elif n == 4:
        s = macaulay2('''
        R = QQ[w..z]; 
    ...
    print('computing lifts of degree ' + str((n-1)* fdegree - n+d) + ' using sage')
    for part in Partitions((n-1)* fdegree -n+d, max_length=n):
    #       pad with zeros
        part += [0] * (n - len(part))
