...
#########################################################
R.<x_0,x_1,x_2,x_3,y_1,y_2> = QQ[]  # Example ring
gens = R.gens()

n = len(gens) - num_poly - 1  # x-vars: n+1, y-vars: num_poly
...
poly_list = [f] if num_poly == 1 else [f, g]
...
def frobenius(g_in, prec=2):
    ...
    for j in range(prec):
        ...
        coeff = binomial(-dord, j) * binomial(dord + prec - 1, dord + j)
...
def frobenius_on_cohom(i, prec=2):
    ...
    return R(g_temp * (p^(n - 1)) / prod(gens))
...
P1raw = P1raw.replace("{","").replace("}","").replace("^","**").replace(" ","").split("matrix")[1:]
...
Pnraw = Pnraw.replace("{","").replace("}","").replace("^","**").replace(" ","").split("matrix")[1:]
        ...
        if monomial not in lift_dict:
            ...
            for i in range(n + num_poly + 1):
                ...
                for exps_r in ri_dict.keys():
                    ri_dict[exps_r] = ri_dict[exps_r] % p^(2+prec)
...
def Ruv_const_helper(v, g):
    ...
    h = sum(lifted[i] + gens[i]*(lifted[i].derivative(gens[i])) for i in range(n+num_poly+1))
...
def compute_vk(u):
    ...
    for v in P1_pts:
        ...
        # in each component
        k = min((u[i] // v[i] if v[i] != 0 else 999999999) for i in range(len(u)))
        if k > best_k:
            best_k = k
            best_v = v
    if best_k < 0 or best_v is None:
        return None, 0
...
#########################################################
frob_matrix = [[0 for _ in range(size_B)] for __ in range(size_B)]
reduction_dict = {}
...
for i in range(size_B):
    ...
    while mon_list:
        ...
        if mterm not in B:
            if mterm not in reduction_dict:
                ...
                # Sum of partial derivatives
                red_val = sum(lifted_r[i].derivative(gens[i]) for i in range(n + num_poly + 1))
    ...
    for j in range(size_B):
        ...
        pm_order = pole_order(B[j]) + num_poly - 1
...
for row_idx in range(size_B):
    for col_idx in range(size_B):
        if frob_matrix[row_idx][col_idx].valuation(p) < 0:
